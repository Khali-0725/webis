...
> This file replaces the Laravel skeleton's default `CLAUDE.md`, which contained
...
**Workflow for whoever (human or Claude) picks up this project:** before
...
and published as an Artifact for the user
(https://claude.ai/code/artifact/209a5e43-135c-4628-b463-6b1de7bfd6c4,
private to the user) rather than left only in chat, since it's a
...
phase-report docs from before `backend/CLAUDE.md` became the single
...
necessary but not sufficient).** Used Claude in Chrome to actually log in
...
**Manually verified live via Claude in Chrome, post-deploy**: logged in as
...
Verified live via Claude in Chrome against the local dev stack (Laragon
